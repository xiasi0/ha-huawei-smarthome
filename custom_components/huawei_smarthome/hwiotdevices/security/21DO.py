"""Explicit Huawei SmartHome model for the 21DO gas detector."""
...
SERVICE_ALARM = "alarm"
SERVICE_EQUIPMENT_STATUS = "equipmentStatus"
STATE_SERVICES = frozenset({SERVICE_ALARM, SERVICE_EQUIPMENT_STATUS})

GAS_LEAK_KEY = "gas_leak"
BINARY_SENSOR_KEYS = (GAS_LEAK_KEY,)
BINARY_SENSOR_NAMES = {GAS_LEAK_KEY: "Gas leak"}
BINARY_SENSOR_DEVICE_CLASSES = {GAS_LEAK_KEY: "gas"}
SENSOR_KEYS = frozenset({"gas_concentration"})

StateListener = Callable[[], None]


class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 21DO device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("21DO device requires prodId=21DO")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        """Return whether the detector reports a mild or severe gas leak."""

        if key != GAS_LEAK_KEY:
            raise ValueError(f"unsupported 21DO binary sensor: {key}")
        alarm_code = self._int_value(SERVICE_ALARM, "alarm")
        return alarm_code in {2, 3} if alarm_code is not None else None

    @property
    def gas_concentration(self) -> int | None:
        """Return the current combustible-gas concentration in ppm."""
