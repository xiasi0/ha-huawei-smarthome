"""Explicit Huawei SmartHome model for the 113B door sensor."""
...
PROFILE_MODEL = "HO-10ZB"

SERVICE_DOOR = "doorSensor"
STATE_SERVICES = frozenset({SERVICE_DOOR})
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 113B device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("113B device requires prodId=113B")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        ...
        if key != DOOR_KEY:
            raise ValueError(f"unsupported 113B binary sensor: {key}")
