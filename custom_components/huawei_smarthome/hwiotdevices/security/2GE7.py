"""Explicit Huawei SmartHome model for the 2GE7 door sensor."""
...
PROFILE_MODEL = "MIR-MC100"

SERVICE_DOOR = "doorContact"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2GE7 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2GE7 device requires prodId=2GE7")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        ...
        if key != DOOR_KEY:
            raise ValueError(f"unsupported 2GE7 binary sensor: {key}")
        return self._bool_value(SERVICE_DOOR, "status")
