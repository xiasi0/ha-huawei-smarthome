"""Explicit Huawei SmartHome model for the 2GB5 water detector."""
...
SERVICE_WATER_ALARM = "warterAlarm"
SERVICE_BATTERY = "battery"
STATE_SERVICES = frozenset({SERVICE_WATER_ALARM, SERVICE_BATTERY})
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2GB5 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2GB5 device requires prodId=2GB5")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        ...
        if key != WATER_LEAK_KEY:
            raise ValueError(f"unsupported 2GB5 binary sensor: {key}")
        return self._bool_value(SERVICE_WATER_ALARM, "alarm")
