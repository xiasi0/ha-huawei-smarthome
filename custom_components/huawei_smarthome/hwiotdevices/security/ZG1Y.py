"""Explicit Huawei SmartHome model for the ZG1Y presence sensor."""
...
ZONE_SERVICE_BY_KEY = dict(
    zip(ZONE_KEYS, USER_FENCE_EVENT_SERVICES, strict=True)
)
ZONE_NAMES = {
    key: f"Zone {index} presence"
    for index, key in enumerate(ZONE_KEYS, start=1)
}
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one ZG1Y device."""

    prod_id = PROD_ID
    ha_platform = HA_PLATFORM
    state_services = STATE_SERVICES
    binary_sensor_keys = ZONE_KEYS
    binary_sensor_names = ZONE_NAMES
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("ZG1Y device requires prodId=ZG1Y")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        """Return whether one configured user area currently has a person."""

        try:
            service_id = ZONE_SERVICE_BY_KEY[key]
        except KeyError as error:
            raise ValueError(f"unsupported ZG1Y binary sensor: {key}") from error
        return self._bool_value(service_id, "existent")
