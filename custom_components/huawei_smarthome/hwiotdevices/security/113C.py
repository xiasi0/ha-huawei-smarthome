"""Explicit Huawei SmartHome model for the 113C motion sensor."""
...
MOTION_KEY = "motion"
BATTERY_LOW_KEY = "battery_low"
BINARY_SENSOR_KEYS = (MOTION_KEY, BATTERY_LOW_KEY)
BINARY_SENSOR_NAMES = {
    MOTION_KEY: "Motion",
    BATTERY_LOW_KEY: "Low battery",
}
BINARY_SENSOR_DEVICE_CLASSES = {
    MOTION_KEY: "motion",
    BATTERY_LOW_KEY: "battery",
}
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 113C device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("113C device requires prodId=113C")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        """Return one motion or low-battery state."""

        if key == MOTION_KEY:
            return self._bool_value(SERVICE_MOTION, "alarm")
        if key == BATTERY_LOW_KEY:
            return self._bool_value(SERVICE_BATTERY, "lowBattery")
        raise ValueError(f"unsupported 113C binary sensor: {key}")
