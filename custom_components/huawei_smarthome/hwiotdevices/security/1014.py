"""Explicit Huawei SmartHome model for the 1014 door sensor."""
...
PROFILE_MODEL = "HO-09AZB"

SERVICE_DOOR = "doorSensor"
SERVICE_BATTERY = "battery"
STATE_SERVICES = frozenset({SERVICE_DOOR, SERVICE_BATTERY})

DOOR_KEY = "door"
BINARY_SENSOR_KEYS = (DOOR_KEY,)
BINARY_SENSOR_NAMES = {DOOR_KEY: "Door"}
BINARY_SENSOR_DEVICE_CLASSES = {DOOR_KEY: "door"}
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 1014 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID.upper():
            raise ValueError("1014 device requires prodId=1014")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        ...
        if key != DOOR_KEY:
            raise ValueError(f"unsupported 1014 binary sensor: {key}")
        return self._bool_value(SERVICE_DOOR, "state")
