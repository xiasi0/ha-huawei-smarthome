"""Explicit Huawei SmartHome model for the 101g motion sensor."""
...
PROFILE_MODEL = "LH-990ZB"

SERVICE_MOTION = "motionSensor"
SERVICE_BATTERY = "battery"
STATE_SERVICES = frozenset({SERVICE_MOTION, SERVICE_BATTERY})

MOTION_KEY = "motion"
BINARY_SENSOR_KEYS = (MOTION_KEY,)
BINARY_SENSOR_NAMES = {MOTION_KEY: "Motion"}
BINARY_SENSOR_DEVICE_CLASSES = {MOTION_KEY: "motion"}
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 101g device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().lower() != PROD_ID:
            raise ValueError("101g device requires prodId=101g")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        """Return whether the sensor currently reports motion."""

        if key != MOTION_KEY:
            raise ValueError(f"unsupported 101g binary sensor: {key}")
        return self._bool_value(SERVICE_MOTION, "alarm")

    @property
    def battery_level(self) -> int | None:
        """Return the remaining battery capacity as a percentage."""

        value = self._int_value(SERVICE_BATTERY, "capacity")
        if value is None:
            return None
