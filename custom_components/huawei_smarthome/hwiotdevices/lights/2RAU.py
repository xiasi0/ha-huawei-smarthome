"""Explicit Huawei SmartHome model for product 2RAU lights."""
...
PROD_ID = "2RAU"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "立达信"
PROFILE_MODEL = "LC5S-G1-D50HM"
PROFILE_NAME = "立达信智能护眼吸顶灯 光之骄子Pro 65W"
...
LIGHT_MAIN = "main"
LIGHT_AUX = "aux"
LIGHT_KEYS = (LIGHT_MAIN, LIGHT_AUX)
LIGHT_NAMES = {
    LIGHT_MAIN: "Main light",
    LIGHT_AUX: "Auxiliary light",
}

SWITCH_KEY_INDUCTION = "induction"
SWITCH_NAMES = {SWITCH_KEY_INDUCTION: "Automatic sensing"}

SERVICE_SWITCH = "switch"
SERVICE_MAIN_SWITCH = "mainSwitch"
SERVICE_MAIN_BRIGHTNESS = "mainBrightness"
SERVICE_MAIN_CCT = "mainCct"
SERVICE_AUX_SWITCH = "auxSwitch"
SERVICE_AUX_BRIGHTNESS = "auxBrightness"
SERVICE_AUX_CCT = "auxCct"
...
STATE_SERVICES = frozenset(
    {
        SERVICE_SWITCH,
        SERVICE_MAIN_SWITCH,
        SERVICE_MAIN_BRIGHTNESS,
        SERVICE_MAIN_CCT,
        SERVICE_AUX_SWITCH,
        SERVICE_AUX_BRIGHTNESS,
        SERVICE_AUX_CCT,
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2RAU device."""
    ...
    switch_keys = (SWITCH_KEY_INDUCTION,)
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2RAU device requires prodId=2RAU")
    ...
    def is_on(self) -> bool | None:
        return self.light_is_on(LIGHT_MAIN)

    @property
    def brightness(self) -> int | None:
        return self.light_brightness(LIGHT_MAIN)
    ...
    def color_temperature(self) -> int | None:
        return self.light_color_temperature(LIGHT_MAIN)
    ...
    def light_is_on(self, light_key: str) -> bool | None:
        switch_sid = self._switch_service(light_key)
        value = self.value(switch_sid, "on")
        if value is None and light_key == LIGHT_MAIN:
            value = self.value(SERVICE_MAIN_SWITCH, "on")
        if value is None:
            return None
        return value in (True, 1, "1")

    def light_brightness(self, light_key: str) -> int | None:
        value = self._int_value(self._brightness_service(light_key), "brightness")
        if value is None:
            return None
    ...
    def light_rgb_color(self, light_key: str) -> None:
        del light_key
        return None

    def light_color_temperature(self, light_key: str) -> int | None:
        return self._int_value(
            self._cct_service(light_key),
            "colorTemperature",
        )

    def light_colour_mode(self, light_key: str) -> None:
        del light_key
        return None

    def feature_is_on(self, key: str) -> bool | None:
        if key != SWITCH_KEY_INDUCTION:
            raise ValueError(f"unsupported 2RAU switch: {key}")
        value = self.value(SERVICE_INDUCTION_SWITCH, "on")
        if value is None:
            return None
        return value in (True, 1, "1")
    ...
    async def async_turn_on_for_light(
        self,
        light_key: str,
        *,
        ...
        self._check_light_key(light_key)
        if rgb_color is not None:
            raise ValueError("2RAU does not support RGB")
        await self.async_set_service(
            self._switch_service(light_key),
            {"on": 1},
        )
        if brightness is not None:
            await self.async_set_brightness_for_light(light_key, brightness)
        if color_temperature is not None:
            await self.async_set_color_temperature_for_light(
                light_key,
                color_temperature,
            )

    async def async_turn_off_for_light(self, light_key: str) -> None:
        self._check_light_key(light_key)
        await self.async_set_service(
            self._switch_service(light_key),
            {"on": 0},
    ...
    async def async_turn_on(
        ...
        *,
        ...
        await self.async_turn_on_for_light(
            LIGHT_MAIN,
    ...
    async def async_turn_off(self) -> None:
        await self.async_turn_off_for_light(LIGHT_MAIN)

    async def async_set_brightness_for_light(
        self,
        light_key: str,
        value: int,
    ) -> None:
        ...
        await self.async_set_service(
            self._brightness_service(light_key),
            {"brightness": device_value},
        )

    async def async_set_brightness(self, value: int) -> None:
        await self.async_set_brightness_for_light(LIGHT_MAIN, value)

    async def async_set_color_temperature_for_light(
        self,
        light_key: str,
        value: int,
    ) -> None:
    ...
    async def async_set_color_temperature(self, value: int) -> None:
        await self.async_set_color_temperature_for_light(LIGHT_MAIN, value)
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2RAU service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2RAU command rejected: sid={sid} errcode={remote_code}"
                )
    ...
    @staticmethod
    def _check_light_key(light_key: str) -> None:
        if light_key not in LIGHT_KEYS:
            raise ValueError(f"unsupported 2RAU light: {light_key}")

    @staticmethod
    def _switch_service(light_key: str) -> str:
        if light_key == LIGHT_MAIN:
            return SERVICE_SWITCH
        if light_key == LIGHT_AUX:
            return SERVICE_AUX_SWITCH
        raise ValueError(f"unsupported 2RAU light: {light_key}")

    @staticmethod
    def _brightness_service(light_key: str) -> str:
        if light_key == LIGHT_MAIN:
            return SERVICE_MAIN_BRIGHTNESS
        if light_key == LIGHT_AUX:
            return SERVICE_AUX_BRIGHTNESS
        raise ValueError(f"unsupported 2RAU light: {light_key}")

    @staticmethod
    def _cct_service(light_key: str) -> str:
        if light_key == LIGHT_MAIN:
            return SERVICE_MAIN_CCT
        if light_key == LIGHT_AUX:
            return SERVICE_AUX_CCT
        raise ValueError(f"unsupported 2RAU light: {light_key}")
