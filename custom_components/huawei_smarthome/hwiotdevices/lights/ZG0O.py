"""Explicit Huawei SmartHome model for product ZG0O lights."""
...
PROFILE_MODEL = "BER-LD01"
PROFILE_NAME = "华为全屋智能1路RGBCW恒压100W"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one ZG0O device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("ZG0O device requires prodId=ZG0O")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        """Turn on and apply the requested RGB or CCT attributes."""
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported ZG0O service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"ZG0O command rejected: sid={sid} errcode={remote_code}"
                )
