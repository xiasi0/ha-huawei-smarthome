"""Explicit Huawei SmartHome model for product 2F6R lights."""
...
class HuaweiDevice:
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2F6R device requires prodId=2F6R")
    ...
    def brightness(self) -> int | None:
        ...
        return round((value - MIN_BRIGHTNESS) * 255 / (MAX_BRIGHTNESS - MIN_BRIGHTNESS))
    ...
    async def async_turn_on(
        ...
        *,
        ...
        """Turn on and apply the requested brightness."""

        if rgb_color is not None or color_temperature is not None:
            raise ValueError("2F6R does not support RGB or color temperature")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2F6R service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2F6R command rejected: sid={sid} errcode={remote_code}"
                )
