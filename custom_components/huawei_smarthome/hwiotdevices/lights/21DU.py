"""Explicit Huawei SmartHome model for product 21DU lights."""
...
PROFILE_MODEL = "Lightstrip-S"
PROFILE_NAME = "水母云智能灯带"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 21DU device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("21DU device requires prodId=21DU")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 21DU service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"21DU command rejected: sid={sid} errcode={remote_code}"
                )
