"""Explicit Huawei SmartHome model for product 2LNI lights."""
...
PROD_ID = "2LNI"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "易百珑"
PROFILE_MODEL = "ERC2302-HM"
PROFILE_NAME = "易百珑双色温灯带"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2LNI device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2LNI device requires prodId=2LNI")
    ...
    def brightness(self) -> int | None:
        ...
        return round(value * 255 / MAX_BRIGHTNESS)
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2LNI does not support RGB")
    ...
    async def async_set_brightness(self, value: int) -> None:
        ...
        device_value = round(value * MAX_BRIGHTNESS / 255)
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2LNI service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2LNI command rejected: sid={sid} errcode={remote_code}"
                )
