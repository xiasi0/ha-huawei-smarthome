"""Explicit Huawei SmartHome model for product 224K lights."""
...
PROD_ID = "224K"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "欧普照明"
PROFILE_MODEL = "LED-RGB strip-HWZX-SP"
COMMAND_ACK_TIMEOUT = 10.0
SUPPORTED_COLOR_MODES = frozenset({"rgb"})
MIN_BRIGHTNESS = 1
MAX_BRIGHTNESS = 100
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 224K device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("224K device requires prodId=224K")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        """Turn on and apply the requested RGB or brightness."""

        if color_temperature is not None:
            raise ValueError("224K does not support color temperature")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 224K service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"224K command rejected: sid={sid} errcode={remote_code}"
                )
