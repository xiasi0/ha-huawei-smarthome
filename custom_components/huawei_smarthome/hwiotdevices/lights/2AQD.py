"""Explicit Huawei SmartHome model for product 2AQD lights."""
...
PROD_ID = "2AQD"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "达伦"
PROFILE_MODEL = "Z3 Pro"
...
SERVICE_CCT = "cct"
SERVICE_NATURAL = "natural"
SERVICE_NIGHT_MODE = "nightmode"
SERVICE_LAMP_SWITCH = "lampswitch"

FEATURE_NATURAL = "natural_light"
FEATURE_NIGHT_MODE = "night_mode"
SWITCH_KEYS = (FEATURE_NATURAL, FEATURE_NIGHT_MODE)
SWITCH_NAMES = {
    FEATURE_NATURAL: "Natural light",
    FEATURE_NIGHT_MODE: "Night mode",
}
FEATURE_SERVICES = {
    FEATURE_NATURAL: SERVICE_NATURAL,
    FEATURE_NIGHT_MODE: SERVICE_NIGHT_MODE,
}
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2AQD device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2AQD device requires prodId=2AQD")
    ...
    def feature_is_on(self, key: str) -> bool | None:
        """Return one P0 feature switch state."""

        try:
            sid = FEATURE_SERVICES[key]
        except KeyError as error:
            raise ValueError(f"unsupported 2AQD switch: {key}") from error
        return self._bool_value(sid, "on")

    async def async_set_feature(self, key: str, enabled: bool) -> None:
        """Set one P0 feature switch."""
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2AQD does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2AQD service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2AQD command rejected: sid={sid} errcode={remote_code}"
                )
