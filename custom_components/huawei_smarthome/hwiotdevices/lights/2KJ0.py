"""Explicit Huawei SmartHome model for product 2KJ0 lights."""
...
PROD_ID = "2KJ0"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "卡麦照明"
PROFILE_MODEL = "KM032-F03"
...
class HuaweiDevice:
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2KJ0 device requires prodId=2KJ0")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2KJ0 does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2KJ0 service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2KJ0 command rejected: sid={sid} errcode={remote_code}"
                )
