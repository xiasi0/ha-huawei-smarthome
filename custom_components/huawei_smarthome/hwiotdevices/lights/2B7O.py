"""Explicit Huawei SmartHome model for product 2B7O lights."""
...
PROD_ID = "2B7O"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "柏曼光电"
PROFILE_MODEL = "E1303"
PROFILE_NAME = "国王湖餐厅吊灯"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2B7O device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2B7O device requires prodId=2B7O")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2B7O does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2B7O service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2B7O command rejected: sid={sid} errcode={remote_code}"
                )
