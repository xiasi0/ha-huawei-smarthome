"""Explicit Huawei SmartHome model for product 2EO8 lights."""
...
PROD_ID = "2EO8"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "万得彩"
PROFILE_MODEL = "HC-XD-068"
PROFILE_NAME = "熠世华彩情景灯"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2EO8 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2EO8 device requires prodId=2EO8")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2EO8 does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2EO8 service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2EO8 command rejected: sid={sid} errcode={remote_code}"
                )
