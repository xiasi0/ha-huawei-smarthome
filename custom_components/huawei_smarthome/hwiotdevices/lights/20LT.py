"""Explicit Huawei SmartHome model for product 20LT lights."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 20LT device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("20LT device requires prodId=20LT")
    ...
    def colour_mode(self) -> int | None:
        value = self._int_value(SERVICE_COLOUR_MODE, "mode")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        """Turn on and apply the requested RGB or color temperature."""
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 20LT service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"20LT command rejected: sid={sid} errcode={remote_code}"
                )
