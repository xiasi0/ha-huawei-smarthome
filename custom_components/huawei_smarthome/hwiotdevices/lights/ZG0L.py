"""Explicit Huawei SmartHome model for the ZG0L wireless panel."""
...
BUTTON_SERVICE_IDS = tuple(f"button{index}" for index in range(1, 21))
...
SWITCH_NAMES = {
    "switch1": "Logical switch 1",
    "switch2": "Logical switch 2",
    "switch3": "Logical switch 3",
}
...
class WirelessSwitchEvent:
    """One physical key action reported by the ZG0L device."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one ZG0L device."""
    ...
    def __init__(self, descriptor: RemoteDeviceDescriptor, mqtt: HuaweiMqttClient) -> None:
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("ZG0L device requires prodId=ZG0L")
    ...
    def feature_is_on(self, characteristic: str) -> bool | None:
        if characteristic not in self.switch_keys:
            raise ValueError(f"unsupported ZG0L switch: {characteristic}")
    ...
    async def async_set_service(self, sid: str, data: dict[str, Any]) -> None:
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported ZG0L service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"ZG0L command rejected: sid={sid} errcode={remote_code}"
                )
