"""Explicit Huawei SmartHome model for product 21JC lights."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 21JC device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("21JC device requires prodId=21JC")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("21JC does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 21JC service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"21JC command rejected: sid={sid} errcode={remote_code}"
                )
