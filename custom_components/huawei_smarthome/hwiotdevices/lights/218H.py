"""Explicit Huawei SmartHome model for product 218H lights."""
...
PROD_ID = "218H"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "阳光照明"
PROFILE_MODEL = "BPZ600-840-7WZ"
PROFILE_NAME = "阳光照明灵睿智能灯泡RGBWC版"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 218H device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("218H device requires prodId=218H")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 218H service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"218H command rejected: sid={sid} errcode={remote_code}"
                )
