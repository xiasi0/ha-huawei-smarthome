"""Explicit Huawei SmartHome model for product 2I7K lights."""
...
PROFILE_MODEL = "OP-IDSCC0140420B2-01"
PROFILE_NAME = "欧普蓝牙Mesh双色温筒射灯驱动"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2I7K device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2I7K device requires prodId=2I7K")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2I7K does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2I7K service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2I7K command rejected: sid={sid} errcode={remote_code}"
                )
