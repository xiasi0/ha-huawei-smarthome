"""Explicit Huawei SmartHome model for the ZG1L Mondrian switch."""
...
PROD_ID = "ZG1L"
PROFILE_MODEL = "PCP-MLB300"
MANUFACTURER_NAME = "华为"
COMMAND_ACK_TIMEOUT = 10.0

SERVICE_SCENE = "scene"
SCENE_ACTION_FIELDS = {"num": "single"}
BUTTON_ACTIONS = tuple(SCENE_ACTION_FIELDS.values())
CHANNEL_SERVICE_IDS = ("switch1", "switch2", "switch3")
BUTTON_SERVICE_IDS = ("button1", "button2", "button3", "button4")
MODE_SERVICE_IDS = ("mode1", "mode2", "mode3", "mode4")
...
class WirelessSwitchEvent:
    """One physical key action reported by the ZG1L device."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one ZG1L device."""
    ...
    def __init__(self, descriptor: RemoteDeviceDescriptor, mqtt: HuaweiMqttClient) -> None:
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("ZG1L device requires prodId=ZG1L")
    ...
    async def async_select_option(self, key: str, option: str) -> None:
        ...
        try:
            ...
            raise ValueError(f"unsupported ZG1L mode option: {option}") from error
    ...
    async def async_set_service(self, sid: str, data: dict[str, Any]) -> None:
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported ZG1L service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"ZG1L command rejected: sid={sid} errcode={remote_code}"
                )
