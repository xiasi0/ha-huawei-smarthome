"""Explicit Huawei SmartHome model for product 2N8V lights."""
...
PROD_ID = "2N8V"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "霍明科技"
PROFILE_MODEL = "HWX-06B"
PROFILE_NAME = "霍尼韦尔智能护眼吸顶灯U7 60W"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2N8V device."""
    ...
    def __init__(self, descriptor: RemoteDeviceDescriptor, mqtt: HuaweiMqttClient) -> None:
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2N8V device requires prodId=2N8V")
    ...
    async def async_turn_on(self, *, brightness: int | None = None, rgb_color: tuple[int, int, int] | None = None, color_temperature: int | None = None) -> None:
        if rgb_color is not None:
            raise ValueError("2N8V does not support RGB")
    ...
    async def async_set_brightness(self, value: int) -> None:
        ...
        await self.async_set_service(SERVICE_BRIGHTNESS, {"brightness": round(value * 100 / 255)})
    ...
    async def async_set_service(self, sid: str, data: dict[str, Any]) -> None:
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2N8V service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(f"2N8V command rejected: sid={sid} errcode={remote_code}")
