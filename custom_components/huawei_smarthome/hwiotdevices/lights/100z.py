"""Explicit Huawei SmartHome model for product 100z lights."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().lower() != PROD_ID:
            raise ValueError("100z device requires prodId=100z")
    ...
    def manufacturer(self) -> str:
        return self._descriptor.manufacturer or "上海三思"
    ...
    def brightness(self) -> int | None:
        return self._int_value(SERVICE_BRIGHTNESS, "brightness")
    ...
    async def async_set_rgb(self, value: tuple[int, int, int]) -> None:
        ...
        await self.async_set_service(SERVICE_COLOUR_MODE, {"mode": 0})
        await self.async_set_service(
            SERVICE_COLOUR,
            dict(zip(("red", "green", "blue"), value)),
        )

    async def async_set_color_temperature(self, value: int) -> None:
        self._check_range(value, 2000, 6500, "color temperature")
        await self.async_set_service(SERVICE_COLOUR_MODE, {"mode": 1})
        await self.async_set_service(
            SERVICE_CCT,
    ...
    async def async_set_service(
        ...
        if sid not in SUPPORTED_SERVICES:
            raise ValueError(f"unsupported 100z service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"100z command rejected: sid={sid} errcode={remote_code}"
                )
