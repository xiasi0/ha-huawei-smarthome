"""Explicit Huawei SmartHome model for the 20SZ rotary scene panel."""
...
PROD_ID = "20SZ"
PROFILE_MODEL = "BL8620RC1-D1"
PROFILE_NAME = "四键旋钮遥控器尊享版"
MANUFACTURER_NAME = "华为"

SERVICE_SCENE = "scene"
SERVICE_ROTATE = "rotate"
STATE_SERVICES = frozenset({SERVICE_SCENE, SERVICE_ROTATE})

SCENE_ACTION = "scene"
ROTATE_ACTION = "rotate"
BUTTON_ACTIONS = (SCENE_ACTION, ROTATE_ACTION)
BUTTON_EVENT_NAMES = {
    SCENE_ACTION: "Scene",
    ROTATE_ACTION: "Rotate",
}
ROTATE_NAMES = {1: "Left rotate", 2: "Right rotate"}
...
class WirelessSwitchEvent:
    """One physical action reported by the 20SZ device."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 20SZ device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("20SZ device requires prodId=20SZ")
    ...
    def handle_mqtt_message(self, topic: str, payload: bytes) -> bool:
        ...
        for service in services:
            ...
            if not is_older_remote_timestamp(
                timestamp,
                self._state_timestamps.get(sid),
            ):
                if sid == SERVICE_SCENE:
                    events.extend(self._parse_scene_events(data, timestamp))
                elif sid == SERVICE_ROTATE:
                    events.extend(self._parse_rotate_events(data, timestamp))
    ...
    def _parse_scene_events(
        ...
        key_code = self._int_from_value(data.get("num"))
        if key_code is None or not 1 <= key_code <= 4:
            return []
        return [
            WirelessSwitchEvent(
                action=SCENE_ACTION,
                key_code=key_code,
                button_id=key_code,
                name=f"Scene {key_code}",
                timestamp=timestamp,
            )
        ]

    def _parse_rotate_events(
        self,
        data: Mapping[str, Any],
        timestamp: str | None,
    ) -> list[WirelessSwitchEvent]:
        key_code = self._int_from_value(data.get("num"))
        if key_code not in ROTATE_NAMES:
            return []
        return [
            WirelessSwitchEvent(
