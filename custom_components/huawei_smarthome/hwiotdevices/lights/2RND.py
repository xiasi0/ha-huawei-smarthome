"""Explicit Huawei SmartHome model for product 2RND lights."""
...
PROD_ID = "2RND"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "西顿照明"
PROFILE_MODEL = "SWBH-QY-240W-2"
PROFILE_NAME = "西顿照明T5无线灯带照明驱动"
...
STATE_SERVICES = frozenset({SERVICE_SWITCH, SERVICE_BRIGHTNESS, SERVICE_CCT})
COMMAND_SERVICES = STATE_SERVICES
StateListener = Callable[[], None]


class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2RND device."""
    ...
    def __init__(self, descriptor: RemoteDeviceDescriptor, mqtt: HuaweiMqttClient) -> None:
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2RND device requires prodId=2RND")
    ...
    def is_on(self) -> bool | None:
        ...
        return None if value is None else value in (True, 1, "1")
    ...
    def brightness(self) -> int | None:
        ...
        return round((value - MIN_BRIGHTNESS) * 255 / 99)
    ...
    def handle_mqtt_message(self, topic: str, payload: bytes) -> bool:
        ...
        for service in services:
            ...
            if not isinstance(sid, str) or not isinstance(data, Mapping):
                continue
            changed = self._merge_service_state(sid, data, service.get("ts")) or changed
        if changed:
            self._notify_state_changed()
        return changed

    async def async_turn_on(self, *, brightness: int | None = None, rgb_color: tuple[int, int, int] | None = None, color_temperature: int | None = None) -> None:
        if rgb_color is not None:
            raise ValueError("2RND does not support RGB")
    ...
    async def async_set_brightness(self, value: int) -> None:
        ...
        await self.async_set_service(SERVICE_BRIGHTNESS, {"brightness": round(1 + value * 99 / 255)})

    async def async_set_color_temperature(self, value: int) -> None:
        self._check_range(value, MIN_COLOR_TEMP_KELVIN, MAX_COLOR_TEMP_KELVIN, "color temperature")
        await self.async_set_service(SERVICE_CCT, {"colorTemperature": value})

    async def async_set_service(self, sid: str, data: dict[str, Any]) -> None:
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2RND service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(f"2RND command rejected: sid={sid} errcode={remote_code}")

    def _handle_command_ack(self, body: Mapping[str, Any], header: Mapping[str, Any]) -> bool:
        target = header.get("from")
        future = self._pending_acks.get(target) if isinstance(target, str) else None
        if future is None or future.done():
            return False
        try:
            future.set_result(int(body.get("errcode")))
