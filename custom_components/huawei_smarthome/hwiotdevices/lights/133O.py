"""Explicit Huawei SmartHome model for product 133O lights."""
...
PROFILE_MODEL = "MQ2.5AD-LED4"
PROFILE_NAME = "FSL智能调光调色筒灯"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 133O device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("133O device requires prodId=133O")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("133O does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 133O service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"133O command rejected: sid={sid} errcode={remote_code}"
                )
