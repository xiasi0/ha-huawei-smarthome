"""Explicit Huawei SmartHome model for product ZG1H lights."""
...
PROFILE_MODEL = "BER-LD10"
PROFILE_NAME = "华为全屋智能射灯筒灯驱动器 - 9W"
...
SERVICE_SUB_PRODUCT = "subProduct"
SERVICE_SWITCH = "switch"
SERVICE_BRIGHTNESS = "brightness"
SERVICE_CCT = "cct"
SERVICE_FAULT = "faultDetection"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one ZG1H device."""
    ...
    binary_sensor_keys = (FAULT_KEY,)
    binary_sensor_names = {FAULT_KEY: "Fault"}
    binary_sensor_device_classes = {FAULT_KEY: "problem"}
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("ZG1H device requires prodId=ZG1H")
    ...
    def fault_code(self) -> int | None:
        return self._int_value(SERVICE_FAULT, "code")

    @property
    def sub_product_id(self) -> str | None:
        value = self.value(SERVICE_SUB_PRODUCT, "subProductId")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        """Return whether the driver reports a fault."""

        if key != FAULT_KEY:
            raise ValueError(f"unsupported ZG1H binary sensor: {key}")
        value = self.value(SERVICE_FAULT, "status")
        if value is None:
            return None
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("ZG1H does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported ZG1H service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"ZG1H command rejected: sid={sid} errcode={remote_code}"
                )
