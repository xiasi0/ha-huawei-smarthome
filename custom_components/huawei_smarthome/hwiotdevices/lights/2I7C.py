"""Explicit Huawei SmartHome model for product 2I7C lights."""
...
PROFILE_MODEL = "OP-DY220/200-24CV/2-WB"
PROFILE_NAME = "欧普蓝牙Mesh双色温灯带驱动"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2I7C device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2I7C device requires prodId=2I7C")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        if rgb_color is not None:
            raise ValueError("2I7C does not support RGB")
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2I7C service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2I7C command rejected: sid={sid} errcode={remote_code}"
                )
