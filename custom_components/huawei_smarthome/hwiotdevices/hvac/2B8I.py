"""Explicit Huawei SmartHome model for the 720 1i air purifier."""
...
PROD_ID = "2B8I"
HA_PLATFORM = "fan"
FAN_ENTITY_NAME = "Air purifier"
MANUFACTURER_NAME = "720"
PROFILE_MODEL = "KJ350F-C350"
COMMAND_ACK_TIMEOUT = 10.0

SERVICE_SWITCH = "switch"
SERVICE_MODE = "mode"
SERVICE_CHILD_LOCK = "childLockSwitch"
SERVICE_PM25 = "pm25"
SERVICE_FAN = "fan"
SERVICE_FILTER = "filterElement1"

# faultDetection and filterElement1/alarm are intentionally not integrated.
...
SWITCH_KEYS = ("child_lock",)
SWITCH_NAMES = {"child_lock": "Child lock"}
SWITCH_FIELDS = {"child_lock": (SERVICE_CHILD_LOCK, "on")}

SENSOR_KEYS = frozenset(
    {"pm2p5", "filter_remaining", "filter_remaining_time"}
)
EXCLUDED_FIELDS = {SERVICE_FILTER: frozenset({"alarm"})}

StateListener = Callable[[], None]


class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2B8I device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2B8I device requires prodId=2B8I")
        self._descriptor = descriptor
        self._mqtt = mqtt
        self._state: dict[str, dict[str, Any]] = {
            sid: self._filtered_service_data(sid, service.data)
    ...
    def percentage(self) -> int | None:
        ...
        if gear is None or not 1 <= gear <= 4:
            return None
        return gear * self.percentage_step

    @property
    def pm2p5(self) -> int | None:
        return self._int_value(SERVICE_PM25, "current")

    @property
    def filter_remaining(self) -> int | None:
        value = self._int_value(SERVICE_FILTER, "leftPer")
        if value is None:
            return None
        return min(max(value, 0), 100)

    @property
    def filter_remaining_time(self) -> int | None:
        value = self._int_value(SERVICE_FILTER, "leftTime")
        if value is None:
            return None
        return max(value, 0)

    def feature_is_on(self, key: str) -> bool | None:
        """Return the state of one writable 2B8I switch feature."""

        try:
            sid, field = SWITCH_FIELDS[key]
        except KeyError as error:
            raise ValueError(f"unsupported 2B8I switch: {key}") from error
    ...
    def update_descriptor(self, descriptor: RemoteDeviceDescriptor) -> None:
        """Update metadata without replacing newer MQTT state."""
        ...
        for sid, service in descriptor.service_states.items():
            ...
            if sid not in self._state:
                self._state[sid] = self._filtered_service_data(sid, service.data)
    ...
    def handle_mqtt_message(self, topic: str, payload: bytes) -> bool:
        ...
        for service in services:
            ...
            if not isinstance(sid, str) or not isinstance(data, Mapping):
                continue
            data = self._filtered_service_data(sid, data)
            if not data:
                continue
            changed = (
    ...
    async def async_set_preset_mode(self, preset_mode: str) -> None:
        try:
            ...
            raise ValueError(f"unsupported 2B8I preset mode: {preset_mode}") from error
    ...
    async def async_set_percentage(self, percentage: int) -> None:
        ...
        gear = min(max(round(percentage / self.percentage_step), 1), 4)
    ...
    async def async_set_service(
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2B8I service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2B8I command rejected: sid={sid} errcode={remote_code}"
                )
    ...
    @staticmethod
    def _filtered_service_data(
        sid: str,
        data: Mapping[str, Any],
    ) -> dict[str, Any]:
        excluded = EXCLUDED_FIELDS.get(sid, ())
        return {
            str(name): value
