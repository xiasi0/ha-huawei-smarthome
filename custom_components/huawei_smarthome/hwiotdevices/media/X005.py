"""Explicit Huawei SmartHome model for the X005 smart speaker."""
...
PROFILE_MODEL = "AIS-BW50-00"
PROFILE_NAME = "华为 AI 音箱 2（电池版）"
...
# The profile contains enumVal=2 for both previous and next. Treat the
# second duplicate as the likely missing value 3 until an outbound trace
# confirms the vendor mapping.
PLAY_CONTROL_PREVIOUS = 2
PLAY_CONTROL_NEXT = 3

SENSOR_KEYS = frozenset({"battery_level", "speaker_state"})
BINARY_SENSOR_KEYS = frozenset({"charging"})
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one X005 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("X005 device requires prodId=X005")
    ...
    def battery_level(self) -> int | None:
        """Return battery.voltage, which the device reports as a percent."""

        value = self._int_value(SERVICE_BATTERY, "voltage")
    ...
    def binary_sensor_is_on(self, key: str) -> bool | None:
        """Return one supported binary feature state."""

        if key != "charging":
            return None
        value = self.value(SERVICE_BATTERY, "charge")
        if value is None:
            return None
    ...
    async def async_set_volume_level(self, level: float) -> None:
        ...
        limit = self._int_value(SERVICE_BATTERY, "restrictMaxVolume")
        if limit is not None and limit >= 0:
            volume = min(volume, min(max(limit, 0), 100))
        await self.async_set_service({"volume": volume})
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported X005 service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"X005 command rejected: sid={sid} errcode={remote_code}"
                )
