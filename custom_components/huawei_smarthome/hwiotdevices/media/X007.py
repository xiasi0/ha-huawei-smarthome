"""Explicit Huawei SmartHome model for the X007 HUAWEI Sound speaker."""
...
PROFILE_MODEL = "AIS-BW80-00"
PROFILE_NAME = "HUAWEI Sound"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one X007 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("X007 device requires prodId=X007")
    ...
    def speaker_state(self) -> str | None:
        ...
        if value is None:
            value = self._int_value(SERVICE_SPEAKER_STATE, "State")
    ...
    def media_metadata(self) -> Mapping[str, Any]:
        """Decode optional nested audioplayer metadata when reported."""
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported X007 service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"X007 command rejected: sid={sid} errcode={remote_code}"
                )
