"""Explicit Huawei SmartHome model for the 001T smart speaker."""
...
PROFILE_MODEL = "BSZHWFA001"
PROFILE_NAME = "华为 AI 音箱"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 001T device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("001T device requires prodId=001T")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 001T service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"001T command rejected: sid={sid} errcode={remote_code}"
                )
