"""Explicit Huawei SmartHome model for product 140B curtain motors."""
...
MANUFACTURER_NAME = "巴百信息"
PROFILE_MODEL = "CM_190901_V01"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 140B device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("140B device requires prodId=140B")
    ...
    def value(self, sid: str, name: str) -> Any:
        """Return one cached 140B characteristic."""
    ...
    def update_descriptor(self, descriptor: RemoteDeviceDescriptor) -> None:
        """Update metadata without replacing newer 140B state."""
    ...
    async def async_set_service(
        ...
        """Publish one hard-coded 140B service command and await its ACK."""

        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 140B service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"140B command rejected: sid={sid} errcode={remote_code}"
                )
