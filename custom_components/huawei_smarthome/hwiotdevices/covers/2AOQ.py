"""Explicit Huawei SmartHome model for product 2AOQ Dooya DH6 curtains."""
...
STATE_SERVICES = frozenset({SERVICE_MODE, SERVICE_OPENER})
COMMAND_SERVICES = frozenset({SERVICE_MODE, SERVICE_OPENER})

StateListener = Callable[[], None]


class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2AOQ device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2AOQ device requires prodId=2AOQ")
    ...
    def mode(self) -> int | None:
        return self._int_value(SERVICE_MODE, "mode")
    ...
    def value(self, sid: str, name: str) -> Any:
        """Return one cached 2AOQ characteristic."""
    ...
    def update_descriptor(self, descriptor: RemoteDeviceDescriptor) -> None:
        """Update metadata without replacing newer 2AOQ state."""
    ...
    async def async_open_cover(self) -> None:
        """Open the DH6 curtain with the hard-coded mode service."""

        await self.async_set_service(SERVICE_MODE, {"mode": MODE_OPEN})

    async def async_close_cover(self) -> None:
        """Close the DH6 curtain with the hard-coded mode service."""

        await self.async_set_service(SERVICE_MODE, {"mode": MODE_CLOSE})

    async def async_stop_cover(self) -> None:
        """Pause the DH6 curtain motor with the hard-coded mode service."""

        await self.async_set_service(SERVICE_MODE, {"mode": MODE_PAUSE})

    async def async_set_cover_position(self, position: int) -> None:
        """Set the DH6 target opening percentage."""
    ...
    async def async_set_service(
        ...
        """Publish one hard-coded 2AOQ service command and await its ACK."""

        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2AOQ service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2AOQ command rejected: sid={sid} errcode={remote_code}"
                )
