"""Explicit Huawei SmartHome model for the 2OJQ smart socket."""
...
PROD_ID = "2OJQ"
HA_PLATFORM = "switch"
MANUFACTURER_NAME = "深圳市港兴达智能科技有限公司"
PROFILE_MODEL = "CP1-HW-A"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2OJQ device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2OJQ device requires prodId=2OJQ")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2OJQ command rejected: errcode={remote_code}"
                )
