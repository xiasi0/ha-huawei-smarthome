"""Explicit Huawei SmartHome model for the 2JO1 smart socket."""
...
PROD_ID = "2JO1"
HA_PLATFORM = "switch"
MANUFACTURER_NAME = "深圳市酷客智能科技有限公司"
PROFILE_MODEL = "CP1-HW"
...
SENSOR_KEYS = frozenset(
    {"power", "electric_current", "total_electricity"}
)
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2JO1 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2JO1 device requires prodId=2JO1")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2JO1 command rejected: errcode={remote_code}"
                )
