"""Explicit Huawei SmartHome model for the 2KD9 wall socket."""
...
STATE_SERVICES = frozenset(
    {SERVICE_SWITCH, SERVICE_POWER, SERVICE_ELECTRIC}
)
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2KD9 device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2KD9 device requires prodId=2KD9")
    ...
    def power(self) -> int | None:
        """Return real-time power in watts."""
    ...
    def total_electricity(self) -> int | float | None:
        """Return the profile-reported total electricity in kWh."""

        return self._number_value(SERVICE_ELECTRIC, "totalConsum")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2KD9 command rejected: errcode={remote_code}"
                )
