"""Explicit Huawei SmartHome model for the 2BSN socket night light."""
...
PROD_ID = "2BSN"
HA_PLATFORM = "light"
MANUFACTURER_NAME = "汇唐光电"
PROFILE_MODEL = "JLTNLP01"
COMMAND_ACK_TIMEOUT = 10.0

SERVICE_SWITCH = "switch"
SERVICE_NIGHT_LIGHT_SWITCH = "NightLightSwitch"
SERVICE_NIGHT_LIGHT_BRIGHTNESS = "NightLightBrightness"
SERVICE_POWER = "power"
SERVICE_CONSUMPTION = "consumption"

FEATURE_SOCKET = "socket"
SWITCH_KEYS = (FEATURE_SOCKET,)
SWITCH_NAMES = {FEATURE_SOCKET: "Socket"}
SENSOR_KEYS = frozenset({"power", "energy_consumption"})
STATE_SERVICES = frozenset(
    {
        SERVICE_SWITCH,
        SERVICE_NIGHT_LIGHT_SWITCH,
        SERVICE_NIGHT_LIGHT_BRIGHTNESS,
        SERVICE_POWER,
        SERVICE_CONSUMPTION,
    }
)
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 2BSN device."""

    prod_id = PROD_ID
    ha_platform = HA_PLATFORM
    light_key = "night_light"
    light_name = "Night light"
    state_services = STATE_SERVICES
    switch_keys = SWITCH_KEYS
    switch_names = SWITCH_NAMES
    sensor_keys = SENSOR_KEYS
    supported_color_modes = frozenset({"brightness"})
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("2BSN device requires prodId=2BSN")
    ...
    def is_on(self) -> bool | None:
        return self._bool_value(SERVICE_NIGHT_LIGHT_SWITCH, "on")
    ...
    def brightness(self) -> int | None:
        ...
        if value is None:
            return None
        return round(min(max(value, 0), 100) * 255 / 100)
    ...
    @property
    def energy_consumption(self) -> int | None:
    ...
    def feature_is_on(self, characteristic: str) -> bool | None:
        """Return the socket output state for the switch projection."""

        if characteristic != FEATURE_SOCKET:
            raise ValueError(f"unsupported 2BSN switch: {characteristic}")
    ...
    async def async_turn_on(
        ...
        *,
        ...
        """Turn on the night light and optionally set brightness."""

        if rgb_color is not None or color_temperature is not None:
            raise ValueError("2BSN night light supports brightness only")
        await self.async_set_service(SERVICE_NIGHT_LIGHT_SWITCH, {"on": 1})
    ...
    async def async_turn_off(self) -> None:
        ...
        await self.async_set_service(SERVICE_NIGHT_LIGHT_SWITCH, {"on": 0})

    async def async_set_brightness(self, value: int) -> None:
        """Set night-light brightness from Home Assistant's 0-255 scale."""

        self._check_range(value, 0, 255, "brightness")
        device_value = round(value * 100 / 255)
        await self.async_set_service(
            SERVICE_NIGHT_LIGHT_BRIGHTNESS,
            {"NightLightBrightness": device_value},
        )

    async def async_set_feature(self, characteristic: str, enabled: bool) -> None:
        """Set the main socket output."""
    ...
    async def async_set_service(self, sid: str, data: dict[str, Any]) -> None:
        ...
        if sid not in COMMAND_SERVICES:
            raise ValueError(f"unsupported 2BSN service: {sid}")
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"2BSN command rejected: sid={sid} errcode={remote_code}"
                )
