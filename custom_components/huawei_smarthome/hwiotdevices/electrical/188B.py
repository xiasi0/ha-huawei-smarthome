"""Explicit Huawei SmartHome model for the 188B smart socket."""
...
PROD_ID = "188B"
HA_PLATFORM = "switch"
MANUFACTURER_NAME = "维尔森科技"
PROFILE_MODEL = "VE-AC02W"
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 188B device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("188B device requires prodId=188B")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"188B command rejected: errcode={remote_code}"
                )
