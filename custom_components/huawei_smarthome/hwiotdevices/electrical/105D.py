"""Explicit Huawei SmartHome model for the 105D receiver controller."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 105D device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("105D device requires prodId=105D")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"105D command rejected: errcode={remote_code}"
                )
