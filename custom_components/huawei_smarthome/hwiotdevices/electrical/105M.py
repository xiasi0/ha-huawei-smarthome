"""Explicit Huawei SmartHome model for the 105M smart socket."""
...
class HuaweiDevice(HuaweiDeviceStateMixin):
    """Runtime context and protocol mapping for one 105M device."""
    ...
    def __init__(
        ...
        if (descriptor.prod_id or "").strip().upper() != PROD_ID:
            raise ValueError("105M device requires prodId=105M")
    ...
    async def async_set_service(self, data: dict[str, Any]) -> None:
        ...
        async with self._command_lock:
            ...
            if remote_code != 0:
                raise RuntimeError(
                    f"105M command rejected: errcode={remote_code}"
                )
