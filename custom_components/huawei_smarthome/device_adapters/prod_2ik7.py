"""User-contributed protocol for Huawei product 2IK7.

Device: 企一照明 低压双色温灯带蓝牙驱动 / Qiyi Lighting low-voltage
dual-colour-temperature strip light drive (deviceModel ``QYHW80B``,
manufacturer ``企一``, protocolType ``Mesh``).
Profile: https://smarthome-drcn.dbankcdn.com/device/guide/2IK7/2IK7.json

Exposed entity:

* ``light`` "设备名" <- ``switch.on``
                    / ``brightness.brightness``    (int 1..100 %)
                    / ``cct.colorTemperature``     (int 2700..6000 K)
...
* ``toggleSwitch.toggle`` ("开关翻转") — the vendor H5 bundle never calls
  ...
  ``brightness.brightness`` and ``cct.colorTemperature``) *and* the card in
  ``data/index.data.js`` ships hard-disabled (``"disabled": {"value": true}``
  on the card and both of its states), so the vendor app does not offer it
  either.  Its write semantics would also be a stateless toggle that conflicts
  with HA's state-driven ``light.toggle``.  Following the "no entity over a
  guessed mapping" rule it stays out; users toggle via the light entity
  itself.
* ``relBrightness`` (accessory-knob calibration flow, zero H5 references)
  — same rationale as the 2RND strip drive adapter.
...
def _clamp_to_profile(value: Any, field: Mapping[str, Any]) -> int | float:
    ...
    if number is None or value_range is None:
        raise ValueError("2IK7 Profile range is incomplete")
...
def _ha_brightness_to_device(
    ...
    value_range = _profile_range(field)
    if number is None or value_range is None:
        raise ValueError("2IK7 brightness range is missing from the Profile")
    minimum, maximum = value_range
...
async def _turn_on(context: DeviceContext, data: Mapping[str, Any]) -> None:
    ...
    if data.get("brightness") is not None:
        ...
        if field is None:
            raise ValueError("2IK7 brightness field is missing from the Profile")
    ...
    if data.get("color_temp_kelvin") is not None:
        ...
        if field is None:
            raise ValueError("2IK7 colour temperature field is missing")
        await context.async_send_service(
            _CCT_SID,
...
class Product2ik7Adapter:
    """Keep all 2IK7 entity and command choices in this file."""
