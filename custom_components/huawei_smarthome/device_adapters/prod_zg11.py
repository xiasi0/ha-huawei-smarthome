"""Product adapter for the OSLO-SS3 three-key smart switch (ZG11, 智能开关 3键).

Sibling of the OSLO-SS2 two-key switch (ZG10) — same vendor H5 bundle
generation (``ZG11/h5_001``; the 2024-style bundle is shared by ZG11/ZG1V
3-key, ZG1W 4-key and ZG2K 6-key; its init comment literally says
"初始化 - 2024-01-25 - ZG11").  Like ZG10, the vuex ``setDevInfo`` action
serialises ``{sid, cid, value}`` into the wire form ``{[sid]: {[cid]: value}}``.

All decisions mirror the ZG10 adapter (same family, same evidence types):

* ``switch1/2/3.on`` — the three relay outputs.  The home card toggles with
  ``valueExchange`` (a plain 0/1 flip); HA sends the explicit target value.
* ``mode1/2/3.mode`` (1 开关模式 / 2 场景模式) — the 按键管理 page's
  ``onRadioChange`` writes ``{modeN:{mode:N}}``.  The H5 i18n names the
  values 开关模式/场景模式; the Profile descCh says 联动模式 for 2 — the
  H5 label is used.
* ``childLockSwitch.on`` — 按键锁定 toggle (i18n ``childLock``).
* ``backlight.on`` — 指示灯 toggle (i18n ``back_light`` = 指示灯).
* ``brightness.brightness`` — 指示灯亮度 slider, H5 ``range:[1,100]`` with
  unit ``%``.  The Profiles of all four sibling products (ZG11/ZG1V/ZG1W/
  ZG2K) declare 0..20 with no unit, contradicting the slider the products
  actually ship; the H5 range is used for the command clamp, reported
  values pass through untouched.
* ``memorySwitch.on`` — 断电记忆 toggle (bool 0/1).
* ``faultDetection.code`` — fault bitmask, decomposed exactly like the H5's
  ``faults[]`` bit mapping (过温保护 bit0, 继电器故障 bit1; the store's
  banner additionally masks ``2 & code`` to surface the relay fault).
* ``faultDetection.status`` (0=正常, 1=异常) — 故障告警 binary_sensor.

Deliberately NOT exposed (same rulings as ZG10):

* ``scene`` — num/DoubleClick/LongClick are event counters read via
  ``getDevHistory``; scene binding is managed by the app's native
  ScenarioManager / gateway commands.
...
* ``reboot`` — the only reset path is ``{reboot:{action:2, devList:[{sn}]}}``
  to the gateway; the descriptor has no ``sn`` and degrading to
  ``action:0`` would reboot the whole gateway.
...
class ProductZG11Adapter:
    """OSLO-SS3 three-key smart switch (ZG11)."""
